...
for (var row = 0; row < INVENTORY_ROWS; row++) {
    for (var col = 0; col < INVENTORY_COLS; col++) {
        ...
        if (i == hovered_slot) // Hovered or is the drag source
            slot_col = make_color_rgb(245, 232, 193);

        if (drag_slot != -1 && i == drag_slot)
            slot_col = make_color_rgb(204, 137, 98);
        ...
        if (slot.sprite != "" && i != drag_slot) {
            var item_spr = asset_get_index(slot.sprite);
            if (item_spr != -1) {
...
if (drag_slot != -1) {
    var ghost_spr = asset_get_index(drag_item.sprite);
    if (ghost_spr != -1) {
    ...
    if (drag_item.count > 1) {
        draw_set_halign(fa_right);
        draw_set_valign(fa_bottom);
        draw_set_color(c_black);
        draw_text(drag_x + slot_w / 2 - 1, drag_y + slot_h / 2 + 1, string(drag_item.count));
        draw_set_color(c_yellow);
        draw_text(drag_x + slot_w / 2 - 1, drag_y + slot_h / 2,     string(drag_item.count));
        draw_set_halign(fa_left);
        draw_set_valign(fa_top);
    }
}
