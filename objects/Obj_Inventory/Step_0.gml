//  Features:
//    • Left-click once       – pick up stack; left-click again to place/merge/swap
//    • Right-click (idle)    – split stack in half
//    • Right-click drag      – paint one item per slot while holding right-click
//                              (paint stroke survives leaving and re-entering slots)
//    • Double left-click     – gather all matching stacks into one slot
//    • I / Escape            – close inventory (returns carried item first)
...
//  RIGHT-CLICK DRAG  –  paint one item per slot
//
//  The painted-slots list is only cleared when right-click is FIRST pressed.
//  Moving outside the inventory grid and back does NOT reset the stroke.
...
if (rclick_painting && mouse_check_button(mb_right)) {
...
// Double-click gather, then pick-up / place
if (mouse_check_button_pressed(mb_left) && !rclick_painting) {
...
// Gather all matching items into slot
    if (_is_double_click) {
