...
function scr_Damage(_target, _amount, _attacker, _apply_knockback = true) {
    ...
    if (_apply_knockback) {
		var delta_x = _target.x - _attacker.x;
		var delta_y = _target.y - _attacker.y;
		show_debug_message($"Delta X: {delta_x}, Delta Y: {delta_y}");
		if (abs(delta_x) > abs(delta_y)) {
			_target.kb_x = sign(delta_x);
			_target.kb_y = 0;
		} else {
			_target.kb_y = sign(delta_y);
			_target.kb_x = 0;
		}
