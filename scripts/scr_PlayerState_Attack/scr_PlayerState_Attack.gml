function scr_PlayerState_Attack() {
    with(obj_XieLian) {
        ...
        attack_sprite_pair = pointer_null;

        if (attack_started != true) {
            switch (sprite_index) {
                case spr_XieLian_WalkDown:
                    attack_sprite_pair = select_attack_sprite(attack_direction.DOWN);
                    break;
                case spr_XieLian_WalkRight:
                    attack_sprite_pair = select_attack_sprite(attack_direction.RIGHT);
                    break;
                case spr_XieLian_WalkLeft:
                    attack_sprite_pair = select_attack_sprite(attack_direction.LEFT);
                    break;
                case spr_XieLian_WalkUp:
                    attack_sprite_pair = select_attack_sprite(attack_direction.UP);
                    break;
            }

            if (attack_sprite_pair != pointer_null) {
                sprite_index = attack_sprite_pair.base;
                image_index = 0;

                /// @type {Id.Instance.obj_XieLianAttackHitbox}
                var attack_hitbox = instance_create_depth(x, y, depth, obj_XieLianAttackHitbox);
                attack_hitbox.sprite_index = attack_sprite_pair.hurt_box;

                // Check if sprite_index contains the word "combo" and adjust the hitbox damage
